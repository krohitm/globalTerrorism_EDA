...
```{r}
global_attacks = rename(global_attacks, region=nation)

world_attacks <- right_join(global_attacks, world, by="region")
world_attacks$Total[which(is.na(world_attacks$Total))]=0

g3 <- ggplot(data = world, aes(x = long, y = lat, group = group)) + 
  coord_fixed(1.3) +
  ditch_the_axes+
  theme(legend.position = "none", panel.background = NULL)+
  geom_polygon(data = world_attacks, aes(label = region, fill = Total), color ="white") +
  scale_fill_gradientn(colours=c("#e1eec3","#f05053"))+
  ggtitle("Number of terrorist attacks around the world")

ggplotly(g3, width = 800, height = 480)
```

#Let's look at the number of people killed across the global in terrorist atttacks.
```{r}
global_kills <- global %>% group_by(nation) %>% summarise(Killed = sum(Killed))
global_kills=rename(global_kills, region=nation)

world_kills <- right_join(global_kills, world, by="region")
world_kills$Killed[which(is.na(world_kills$Killed))]=0

g3 <- ggplot(data = world, aes(x = long, y = lat, group = group)) + 
  coord_fixed(1.3) +
  ditch_the_axes+
  theme(legend.position = "none", panel.background = NULL)+
  geom_polygon(data = world_kills, aes(label = region, fill = Killed), color ="white") +
  scale_fill_gradientn(colours=c("#e1eec3","#f05053"))+
  ggtitle("Number of people killed around the world")

ggplotly(g3, width = 800, height = 450)
```
```{r}
global_wound <- global %>% group_by(nation) %>% summarise(wounded = sum(wounded))
global_wound=rename(global_wound, region=nation)

world_wound <- right_join(global_wound, world, by="region")
world_wound$wounded[which(is.na(world_wound$wounded))]=0

g3 <- ggplot(data = world, aes(x = long, y = lat, group = group)) + 
  coord_fixed(1.3) +
  ditch_the_axes+
  theme(legend.position = "none", panel.background = NULL)+
  geom_polygon(data = world_wound, aes(label = region, fill = wounded), color ="white") +
  scale_fill_gradientn(colours=c("#e1eec3","#f05053"))+
  ggtitle("Number of people wounded around the world")
...
visualize_count_map <- function(global_table, title){
  ...
  g8 <- ggplot(data = world, aes(x = long, y = lat, group = group)) + 
  coord_fixed(1.3) +
  ditch_the_axes+
  theme(legend.position = "none", panel.background = NULL)+
  ...
  ggplotly(g8, width = 850, height = 480)
}
```
...
#join the data found with world data
world_int_attacks <- right_join(global_int_attacks, world, by="region")
world_int_attacks$Total[which(is.na(world_int_attacks$Total))]=0
```
...
g8 <- ggplot(data = world, aes(x = long, y = lat, group = group)) + 
  ...
  ditch_the_axes+
  theme(legend.position = "none", panel.background = NULL)+
  geom_polygon(data = world_int_attacks, aes(label = region, fill = Total), color ="white") +
  scale_fill_gradientn(colours=c("#e1eec3","#f05053"))+
  ggtitle("International attacks around the globe")
...
#join the data found with world data
world_dom_attacks <- right_join(global_dom_attacks, world, by="region")
world_dom_attacks$Total[which(is.na(world_dom_attacks$Total))]=0
```

```{r}
g9 <- ggplot(data = world, aes(x = long, y = lat, group = group)) + 
  coord_fixed(1.3) +
  ditch_the_axes+
  theme(legend.position = "none", panel.background = NULL)+
  geom_polygon(data = world_dom_attacks, aes(label = region, fill = Total), color ="white") +
  scale_fill_gradientn(colours=c("#e1eec3","#f05053"))+
  ggtitle("Domestic attacks around the globe")

ggplotly(g9, width = 850, height = 480)
```
